...
def predict_emprestimo(data):
    url = 'https://advisory-tallou-thales-998f7a82.koyeb.app/empresa/predict'
...
with col8:
    posse_casa = st.selectbox('Insira a posse da casa', (['RENT', 'OWN', 'MORTGAGE', 'OTHER']))


col9, col10, col11= st.columns(3)

with col9:
    finalidade_emprestimo = st.selectbox('Insira a Finalidade do Empréstimo', (['PERSONAL', 'EDUCATION', 'MEDICAL', 'VENTURE', 'HOMEIMPROVEMENT',
       'DEBTCONSOLIDATION']))
...
if st.button('Fazer Previsão'):
    with st.spinner('Nosso Modelo de Inteligência Artificial está analisando os dados....'):
        ...
        if previsao != 0:
            st.markdown("<h4 style='color:red;'> Nosso Modelo de Inteligência Artificial recomenda não disponibilizar crédito para esse cliente, pois ele possui uma alta probabilidade de não pagar </h4>", unsafe_allow_html=True)
        else:
            st.markdown("<h4 style='color:green;'> Nosso Modelo de Inteligência Artificial recomenda disponibilizar crédito para esse cliente</h4>", unsafe_allow_html=True)
